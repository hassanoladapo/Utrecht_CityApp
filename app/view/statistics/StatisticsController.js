...
Ext.define('CityApp.view.statistics.StatisticsController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.statistics-main',

    requires: [
        'CityApp.view.statistics.LegendWindow'
    ],
    ...
    initializeView: function(){
        ...
        var Boundary = new ol.layer.Image({
            source: new ol.source.ImageWMS({
                ...
                params: {
                    'MAP': 'd:/iishome/student/s2292440/UtrechtCityApp/MVC.map',
                    'LAYERS': 'boundary',
                    'SERVER': 'MapServer',
                    'TILED': true
                }
        ...
        var districtsWMS = new ol.layer.Image({
            source: new ol.source.ImageWMS({
                url: 'https://gisedu.itc.utwente.nl/cgi-bin/mapserv.exe?',
                params: {
                    'MAP': 'd:/iishome/student/s2292440/UtrechtCityApp/app/api/adminboundaries.map',
                    'LAYERS': 'districts',
                    'SERVER': 'MapServer',
                    'TILED': true
                }
    ...
    renderVectorFeatures: function(){
    	...
    	var featuresUrl = 'https://gisedu.itc.utwente.nl/cgi-bin/mapserv.exe?map='+
         'd:/iishome/student/s2292440/UtrechtCityApp/app/api/adminboundaries.map'+
         '&service=WFS&version=1.1.0&request=GetFeature&typename=cbs:districts'+
         '&outputFormat=geojson&srsname=EPSG:28992&cityname=Utrecht';
        ...
        this.onMouseOverFeature.getFeatures().on('add', function(evt){
        	if (CityApp.app.stsCtrl.userEvent){
	            d3.select("#bar_" + evt.element.get('district_code'))
	                .style("stroke", "#222")
	                .style("stroke-width", 2)
	                .style("fill-opacity", 0.9);

	                var record = CityApp.app.stsCtrl.districts.find(function(r){
                    return r.data.code == evt.element.get('district_code')
                });
                CityApp.app.stsCtrl.refreshPieChart(record.data.landuse_2012, record.data.district_name);
	        };
        });    
        ...
        this.onMouseOverFeature.getFeatures().on('remove', function(evt){
        	if (CityApp.app.stsCtrl.userEvent){
	            d3.select("#bar_" + evt.element.get('district_code'))
	                ...
	                CityApp.app.stsCtrl.refreshPieChart(
                    CityApp.app.stsCtrl.pieChart.cityTotals, 
                    CityApp.app.cityName
                );
            };
        });     
    },
    ...
    drawPieChart: function(districts, chartPanel){
        ...
        function mouseOverPie(slice){
            ...
            sliceData = CityApp.app.stsCtrl.districts.map(function(r){
                return {
                    code: r.data.code,
            ...
            CityApp.app.stsCtrl.showChoropleth(sliceData, slice.data.group, className);
            CityApp.app.stsCtrl.barChart.popBars = false;
            CityApp.app.stsCtrl.redrawBars(
                sliceData, 
        ...
        function mouseOutOfPie(){
            ...
            d3.selectAll('.legend-element')
                .style('opacity', 1);
            CityApp.app.stsCtrl.choroLayer.setStyle(CityApp.app.stsCtrl.defaultStyle);
            CityApp.app.stsCtrl.barChart.popBars = true;
            CityApp.app.stsCtrl.redrawBars(
                CityApp.app.stsCtrl.districts, 
                '#CACCCE', 
                '#AAA', 
                'Number of Inhabitants (2016)'
            );
        ...
        pc.obj.append('text')
            ...
            .text('Landuse in : ' + CityApp.app.cityName);
    ...
    drawBarChart: function(districts, chartPanel){
        ...
        function onMouseOverChart(record){
            ...
            barTooltip.transition()
                ...
                .style("display", "block")
                .style("opacity", 0.9);
            CityApp.app.stsCtrl.refreshPieChart(record.data.landuse_2012, record.data.name);
                /* highlight the corresponding feature on the map */
            var currentFeature = 
                CityApp.app.stsCtrl.choroLayer.getSource().getFeatures().filter(function(f){
                    return f.getProperties().district_code == record.data.code;
                });
            CityApp.app.stsCtrl.userEvent = false;
            CityApp.app.stsCtrl.onMouseOverFeature.getFeatures().push(currentFeature[0]);  
        };
        /*---*/
        ...
        function onMouseOutOfChart(record){
            ...
            barTooltip.transition()
                .duration(1)
                .style("display", "none");
                CityApp.app.stsCtrl.onMouseOverFeature.getFeatures().clear();
                CityApp.app.stsCtrl.userEvent = true;
        };
        /*---*/  
        ...
        bc.obj.append("g")
          ...
          .append("text")
            ...
            .style("font-weight", 400)
            .style("text-anchor", "middle")
            .text("District Codes - (" + CityApp.app.cityName + ")");
        ...
        bc.obj.append("g")
          ...
          .append("text")
            ...
            .style("font-weight", 400)
            .style("text-anchor", "start")
            .text("Number of Inhabitants (2016)");
